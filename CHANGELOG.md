...
**UI Tweaks**: Relocated version tag to the footer of the Data Management page.
...
### Added
**Backend Storage**: Migrated data source of truth from `localStorage` to `server.js` (JSON file storage).
**Automated Backups**: Added a backend system to create daily/weekly JSON backups in `/opt/bill-tracker/backups`.
**Sync API**: Implemented `GET/POST /api/data` endpoints for cross-device synchronization.

## v0.7.2 - Installation Fix
### Fixed
**Updater Logic**: Improved `install.sh` git tag fetching to handle detached states and force updates.
