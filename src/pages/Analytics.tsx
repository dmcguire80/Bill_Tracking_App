                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="space-y-2">
                                        {changedBills.map(bill => (
                                            <div key={bill.templateId} className="flex items-center justify-between text-sm">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`flex items-center gap-1 ${bill.changeType === 'increase' ? 'text-red-400' : 'text-emerald-400'
                                                        }`}>
                                                        {bill.changeType === 'increase' ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                                                        {bill.changePercentage > 0 ? '+' : ''}{bill.changePercentage.toFixed(1)}%
                                                    </span>
                                                </div>
                                                <button
                            ...
                            <tbody className="divide-y divide-white/5">
                                {analytics.length === 0 ? (
                                    ...
                                    analytics.map((bill, idx) => (
                                        <tr key={bill.templateId} className={idx % 2 === 0 ? 'bg-black/10' : 'bg-transparent'}>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-white font-medium">{bill.templateName}</span>
                                                    {bill.hasChange && (
                                                        <span className={`text-xs px-1.5 py-0.5 rounded flex items-center gap-1 ${bill.changeType === 'increase'
                                                            ? 'bg-red-500/10 text-red-400 border border-red-500/20'
                                                            : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                                                            }`}>
                                                            {bill.changeType === 'increase' ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
                                                            {bill.changePercentage > 0 ? '+' : ''}{bill.changePercentage.toFixed(1)}%
                                                        </span>
                                                    )}
                                                </div>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <span className={`font-mono font-semibold ${bill.ytdPaid - bill.ytdPlanned > 0
                                                    ? 'text-red-400'
                                                    : bill.ytdPaid - bill.ytdPlanned < 0
                                                        ? 'text-emerald-400'
                                                        : 'text-neutral-400'
                                                    }`}>
                                                    {bill.ytdPaid - bill.ytdPlanned > 0 ? '+' : ''}${(bill.ytdPaid - bill.ytdPlanned).toFixed(2)}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 text-center">
                                                <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${bill.paidCount === bill.plannedCount
                                                    ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                                                    : bill.paidCount === 0
                                                        ? 'bg-neutral-500/10 text-neutral-400 border border-neutral-500/20'
                                                        : 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20'
                                                    }`}>
                                                    {bill.paidCount === bill.plannedCount ? 'All Paid' : bill.paidCount === 0 ? 'None Paid' : 'In Progress'}
                                                </span>
                                            </td>
                                        </tr>
                                    ))
