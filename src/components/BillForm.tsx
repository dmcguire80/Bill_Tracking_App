...
export const BillForm = ({ initialData, onSave, onClose }: BillFormProps) => {
    ...
    const [amounts, setAmounts] = useState<Record<string, number>>((initialData?.amounts || {}) as Record<string, number>);
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-[#1e293b] border border-white/10 rounded-xl p-6 w-full max-w-md shadow-2xl">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-bold text-white">{initialData ? 'Edit Payment' : 'Add One-Time Payment'}</h2>
