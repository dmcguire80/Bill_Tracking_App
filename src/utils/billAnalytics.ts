...
export const calculateBillAnalytics = (
    ...
    return templates.map(template => {
        ...
        const templateEntries = entries.filter(e => {
            ...
            const yearMatches = e.month.includes(yearSuffix) ||
                e.month.includes(yearShort) ||
                e.month.includes(yearFull);
        ...
        const currentAmount = Object.values(template.amounts).reduce((a, b) => (a || 0) + (b || 0), 0) || 0;
        const ytdPlanned = plannedCount * currentAmount;

        // Calculate average paid amount to detect changes
        const averagePaidAmount = paidEntries.length > 0
            ? ytdPaid / paidEntries.length
            : 0;
